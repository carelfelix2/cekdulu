...
export class AffiliateLinksService {
  ...
  create(data: { productId: string; marketplaceId: string; listingId?: string; url: string; shortCode: string; trackingCode: string }) {
    return this.prisma.affiliateLink.create({ data });
  }

  update(id: string, data: Partial<{ url: string; isPrimary: boolean; utmSource?: string; utmMedium?: string; utmCampaign?: string }>) {
